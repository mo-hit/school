 ...
 * unmodified ("echoed"). For lab 3 it should be modified so that it can accept
 * and process multiple client connections concurrently (completed)
 *
...
#	endif
	...
	while( 1 )
	{
		//initialize read and write sets to 0
		FD_ZERO(&rset);
		FD_ZERO(&wset);

		//add the listen file descriptor to the read set
		FD_SET(listenfd,&rset);
		max_fd = listenfd;

		if (!connections.empty()) {
			//add connections to corresponding set based on state so we know what to do with them later
		...
		// by adding select and modifying the code we should now be able to 
		// use the server as a concurrent server
		int ret = select(max_fd+1,&rset,&wset,0,0);
		...
		#			endif
				...
				// initialize connection data and add to list so we know what to do with it later
